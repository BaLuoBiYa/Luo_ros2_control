...
def generate_launch_description():
    ...
    declare_xacro = DeclareLaunchArgument(
        name = 'xacro_path',
        default_value=get_package_share_directory("legged_bringup") + "/resource" + "/" + robot_type + "/urdf/gazebo.xacro"
    )

    declare_urdfFile = DeclareLaunchArgument(
        name = 'urdfFile',
        default_value=get_package_share_directory("legged_bringup") + "/resource" + "/" + robot_type + "/urdf/A1.urdf"
    )
    declare_referenceFile = DeclareLaunchArgument(
        name = 'referenceFile',
        default_value=get_package_share_directory("legged_bringup") + "/resource" + "/" + robot_type + "/config/reference.info"
    )
    declare_taskFile = DeclareLaunchArgument(
        name = 'taskFile',
        default_value=get_package_share_directory("legged_bringup") + "/resource" + "/" + robot_type + "/config/task.info"
    )
    declare_gaitFile = DeclareLaunchArgument(
        name = 'gaitFile',
        default_value=get_package_share_directory("legged_bringup") + "/resource" + "/" + robot_type + "/config/gait.info"
    )
    ...
    scene_model = get_package_share_directory("legged_sim") + "/scene/model.sdf"
    gz_spawn_scene = Node(
        package='ros_gz_sim',
        executable='create',
        output='screen',
        arguments=['-file', scene_model, 
                   '-name','obstacle',
                   '-z','0.01',
                   '-R','3.141592654'],
    )
    ...
    robot_model = get_package_share_directory("legged_sim") + "/" + robot_type + "/model.sdf"
    gz_spawn_robot = Node(
        package='ros_gz_sim',
        executable='create',
        output='screen',
        arguments=['-file', robot_model, 
                   '-name','A1',
                   '-z','0.35'],
    )
    # 生成机器人
