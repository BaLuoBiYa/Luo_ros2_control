...
namespace legged {
    ...
    ocs2::vector_t FromTopicStateEstimate::update(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
        auto msg = buffer_.try_get();
        if (msg == std::nullopt) {
            return rbdState_;
        }
        nav_msgs::msg::Odometry odom = msg.value();

        updateAngular(
            quatToZyx(Eigen::Quaternion<ocs2::scalar_t>(odom.pose.pose.orientation.w, odom.pose.pose.orientation.x,
                                                        odom.pose.pose.orientation.y, odom.pose.pose.orientation.z)),
            Eigen::Matrix<ocs2::scalar_t, 3, 1>(odom.twist.twist.angular.x, odom.twist.twist.angular.y,
                                                odom.twist.twist.angular.z));
        updateLinear(Eigen::Matrix<ocs2::scalar_t, 3, 1>(odom.pose.pose.position.x, odom.pose.pose.position.y,
                                                         odom.pose.pose.position.z),
                     Eigen::Matrix<ocs2::scalar_t, 3, 1>(odom.twist.twist.linear.x, odom.twist.twist.linear.y,
