...
namespace legged {
    ...
    void WbcBase::updateMeasured(const vector_t &rbdStateMeasured) {
        ...
        for (size_t i = 0; i < info_.numThreeDofContacts; ++i) {
            ...
            pinocchio::getFrameJacobian(model, data, info_.endEffectorFrameIndices[i], pinocchio::LOCAL_WORLD_ALIGNED,
                                        jac);
    ...
    Task WbcBase::formulateFloatingBaseEomTask() {
        ...
        matrix_t a =
            (matrix_t(info_.generalizedCoordinatesNum, numDecisionVars_) << data.M, -j_.transpose(), -s.transpose())
                .finished();
        vector_t b = -data.nle;
